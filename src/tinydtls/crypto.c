...
#else
static ms_handle_t cipher_context_mutex = MS_HANDLE_INVALID;
#endif
#endif
...
#else
  if (cipher_context_mutex == MS_HANDLE_INVALID) {
      ms_mutex_create("dtls_lock", MS_WAIT_TYPE_PRIO, &cipher_context_mutex);
  }
  ms_mutex_lock(cipher_context_mutex, MS_TIMEOUT_FOREVER);
#endif
#endif
...
#else
  ms_mutex_unlock(cipher_context_mutex);
#endif
#endif
}
